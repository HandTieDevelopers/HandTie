...
void MCP4251::write(int address, int value){
...
uint8_t MCP4251::read(int address){
  uint8_t value;
  digitalWrite(slave_select_pin,LOW);
...
unsigned int MCP4251::wiper0_pos()
{
  return (this->read(kADR_WIPER0));
}

unsigned int MCP4251::wiper1_pos()
{
  return (this->read(kADR_WIPER1));
}
...
void MCP4251::wiper0_pos(unsigned int wiper_pos)
{
  this->write(kADR_WIPER0, wiper_pos);
}
