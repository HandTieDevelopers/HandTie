...
Button close;
Button selectModel;
Button selectScalingData;
Button selectOutputFolder;
...
void inputModelFileSelected(File selectedFile) {
  if (selectedFile == null) {
    println("didn't select any file as input file");
  } 
  else {
    modelFileName.setText(selectedFile.getAbsolutePath());
  }
}


void inputScalingFileSelected(File selectedFile) {
  if (selectedFile == null) {
    println("didn't select any file as input file");
  } 
  else {
    scalingFileName.setText(selectedFile.getAbsolutePath());
  }
}
...
void outputDataFolderSelected(File selectedFolder) {
  if(selectedFolder == null) {
    ...
    outputFileName.setText(selectedFolder.getAbsolutePath());
  }
}
...
void setup() {
  ...
  //add Text labels
  modelFileName = controlP5.addTextfield("Model Path", 10, 10, 200, 20); // X,Y,Width,Height
  ...
  int buttonHeight = 20;
  int button_X = 220;
  start = controlP5.addButton("Start", 150, 160, 260, buttonWidth, buttonHeight);
  close = controlP5.addButton("Close", 150, 230, 260, buttonWidth, buttonHeight);
  selectModel = controlP5.addButton("selectModel", 150, button_X, 10, buttonWidth, buttonHeight);
  selectScalingData = controlP5.addButton("selectScalingData", 150, button_X, 50, buttonWidth, buttonHeight);
  selectOutputFolder = controlP5.addButton("selectOutputFolder", 150, button_X, 150, buttonWidth, buttonHeight);
...
void controlEvent(ControlEvent theEvent) {
  if (theEvent.isFrom(selectModel)) {
    selectInput("Select a file to be input model file", "inputModelFileSelected");
  }
  else if (theEvent.isFrom(selectScalingData)) {
    selectInput("Select a file to be input scaling file", "inputScalingFileSelected");
  }
  else if (theEvent.isFrom(selectOutputFolder)) {
    selectFolder("Select a folder to be output directory", "outputDataFolderSelected");  
  }
  else if (theEvent.isFrom(start)) {
